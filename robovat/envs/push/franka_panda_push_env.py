...
class FrankaPandaPushEnv(arm_env.ArmEnv):
    """Pushing task environment."""
    ...
    def __init__(self,
        ...
        super(FrankaPandaPushEnv, self).__init__(
    ...
    def _reset(self):
        """Reset."""
        observations = super(FrankaPandaPushEnv, self)._reset()
    ...
    def _reset_scene(self):
        ...
        super(FrankaPandaPushEnv, self)._reset_scene()
        # Simulation.
        if self.is_simulation:
    ...
    def step(self, action):
        ...
        See parent class.
        """
        observation, reward, done, info = super(FrankaPandaPushEnv, self).step(action)
        ...
        while(self.phase != 'done'):
            ...
            if self._is_phase_ready():
                ...
                if self.phase == 'pre':
                    ...
                    self.table.set_dynamics(lateral_friction=0.1)
                    ...
                    # self.robot.move_to_gripper_pose(pose)
                    self.robot.move_along_gripper_path(waypoints[self.num_waypoints][1:])
                    ...
                    pose.z = self.config.ARM.GRIPPER_SAFE_HEIGHT
                    self.robot.move_to_gripper_pose(pose)
                    self.table.set_dynamics(lateral_friction=1.0)
    ...
    def _compute_waypoints(self, action, num_points=100):
        ...
        tilt_angle = -0.5
        start = Pose(
            [[x, y, z], [np.pi, tilt_angle, (angle + np.pi) % (2 * np.pi) - np.pi]]
        )

        waypoints = [start]
        for i in range(1, num_points):
            delta_x = motion[0] * self.config.ACTION.MOTION.TRANSLATION_X * i / num_points
            delta_y = motion[1] * self.config.ACTION.MOTION.TRANSLATION_Y * i / num_points
            x = x + delta_x
            y = y + delta_y
